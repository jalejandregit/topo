...
public class Scraping {
	...
	private TreeMap<Integer,String> linkProfileName = (TreeMap<Integer,String>) new TreeMap<Integer,String>();
	private TreeMap<Integer,String> linkProfileUrl = (TreeMap<Integer,String>) new TreeMap<Integer,String>();
	//LOG log;

	//public Scraping(Parameters parameters,Set<String> readVisited,List<String> readPagesToVisit) {
	public Scraping(Parameters parameters) {
		super();
		...
		try {
			...
			System.out.println("Scrapping -> Domain:" + domain + " MAX_PAGES_TO_SEARCH=" + MAX_PAGES_TO_SEARCH + " iniUrl=" + url);
			search();
	...
	public boolean crawl(String url) {
		...
		try {
			...
			/* Determinar si se ha de explorar la url: 
			 * -> Si es la url principal no exploramos, aunque si que registramos los links 
			 * -> procesaLink, aplica los filtros de selectores de entradas y exclusiones
			 * */
			
			//if (!Objects.equals(url, new String("https://" + domain)) ) {
			if (!Objects.equals(url, new String(this.url)) ) {
				explora=true;
			...
			for(Element link : linksOnPage)  {
				...
				Element profile = htmlDocument.select("dt.profile-data > a").first();

				if (profile!=null) {
					 profileName  = profile.text();
					 profileUrl  = profile.attr("href");
				}else {
					String[] parts = domain.split("\\.");
					//String part1 = parts[0]; // 123
					//String part2 = parts[1]; // 654321
					//System.err.println("parts:" + part1);
					 profileName  = parts[0];
					 profileUrl  = "https://" + domain; //postLink;
				}
				
				
				//System.out.println("profile:" + urlProfile + "-->"+ profile.text());
				
				
				
				Element description = htmlDocument.select("div.post-body").first();
				...
				if(explora) {
					...
					/* Controlar el tipo de imagen si es de los deseados */
					if(returnTipo!=-1 ){
						image = true;
						String srcThumb="";
			...
			if(imagenes && explora) {
				for (int i = 0; i < postImg.size(); i++) {
					...
					this.linkProfileName.put(ordenPost, profileName);
					this.linkProfileUrl.put(ordenPost, profileUrl);
	...
	public boolean procesaLink(Element link) {
		...
		if(href.contains(domain) && href.indexOf("https://" + domain)==0) {
			//Exclusiones mediante tags
			boolean exclude=params.getExclude();
			boolean include=params.getInclude();
	...
	public TreeMap<Integer, String> getLinkProfileName() {
		return linkProfileName;
	}
	
	public TreeMap<Integer, String> getLinkProfileUrl() {
